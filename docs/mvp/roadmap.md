...
- 当前推荐实施切片：`三单匹配`。它应消费已发布 PO、收货单和供应商发票，继续优先使用 MySQL 和同步服务调用识别数量/金额差异；RabbitMQ 事件触发和 AI 异常解释留给后段增强。
...
| P0 | 三单匹配 | 对 PO、收货、发票做差异识别 | 财务人员、采购员 | 发票金额比 PO 多 2,300，进入异常队列 | PO、收货单、发票三方数量和金额匹配；异常识别；异常队列；处理记录 | MySQL 存储匹配结果、差异项和处理记录；前期由业务服务同步触发匹配，RabbitMQ 事件触发后段增强 | P1 接入异常解释和处理建议 | 建议下一步 |
