...
- 当前推荐实施切片：`PO 采购订单`。它应从 RFQ 中标报价生成采购订单，继续复用 MySQL 和同步服务调用；暂不要求 RabbitMQ 事件发布或真实附件上传。
...
| P0 | PO 采购订单 | 从中标报价生成采购订单 | 采购员、供应商管理员、财务人员 | 选择供应商后生成 PO 并发布 | PO 创建、发布、状态流转、金额和税率记录、交付计划 | MySQL 存储 PO 主数据、明细、税率和状态；暂不要求 RabbitMQ 事件发布 | 暂不涉及 | 建议下一步 |
