...
- 当前推荐实施切片：`真实附件上传`。它应在核心采购闭环和 AI 建议层稳定后，补齐报价单、收货凭证和发票文件的 MinIO 上传、下载和业务单据附件关联能力。
...
| P0 | 收货与发票 | 支持 PO 后续收货、发票录入 | 采购员、仓储人员、财务人员 | 录入部分收货与供应商发票 | 收货登记、发票登记、附件信息记录、数量和金额记录 | MySQL 存储收货单、发票、明细和附件元数据占位；MinIO 后段统一接入发票文件和收货凭证真实上传；暂不要求 RabbitMQ 事件发布 | 暂不涉及 | 已完成并归档：`implement-receipts-and-invoices` |
...
| P1 | 真实附件上传 | 在业务闭环稳定后支持报价、收货和发票文件上传 | 采购员、供应商管理员、仓储人员、财务人员 | 上传报价单、收货凭证和发票文件，并在对应业务单据中查看附件 | MinIO bucket、对象 key、文件元数据、上传/下载接口、大小和类型校验、业务单据附件关联 | MinIO 存储真实文件；MySQL 存储附件元数据和业务关联；覆盖 RFQ、收货和发票附件，不回补采购申请 Intake 附件 | 暂不涉及 | 建议下一步 |
