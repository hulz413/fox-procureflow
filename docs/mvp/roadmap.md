...
- 当前 active OpenSpec change：`implement-approval-workflow`，用于实现审批流。
- 当前推荐实施切片：`审批流`。它应复用已完成的 `requestId`、`companyId`、`requesterId`、`departmentId`、`categoryId`、`budgetAccountId`、`supplierId` 和申请明细数据，不要在审批模块重复创建采购申请；本切片只依赖 MySQL，不引入 Redis、RabbitMQ 或 MinIO。
...
| P0 | 审批流 | 按公司、金额、品类生成审批路径 | 申请人、审批人、采购员 | IT 设备大额采购进入部门负责人 + 财务审批；设备备件采购进入生产负责人审批 | 提交、审批通过、驳回、撤回、审批时间线、审批规则匹配 | MySQL 存储审批实例、节点、记录和上下文快照；待办直接按 MySQL 查询，暂不依赖 Redis | P1 接入审批前风险提示 | 已创建 active change：`implement-approval-workflow` |
| P0 | RFQ 询报价 | 采购员对已审批申请发起询价并比较报价 | 采购员、供应商管理员 | 三家供应商报价，系统展示价格、交期、评分、风险 | RFQ 创建、供应商选择、报价录入、报价对比、推荐排序；附件先记录文件名/说明，暂不做真实上传 | MySQL 存储 RFQ、报价、供应商评分和附件元数据占位；MinIO 后段统一接入真实报价附件 | P1 接入报价推荐解释 | 未开始 |
| P0 | PO 采购订单 | 从中标报价生成采购订单 | 采购员、供应商管理员、财务人员 | 选择供应商后生成 PO 并发布 | PO 创建、发布、状态流转、金额和税率记录、交付计划 | MySQL 存储 PO 主数据、明细、税率和状态；暂不要求 RabbitMQ 事件发布 | 暂不涉及 | 未开始 |
| P0 | 收货与发票 | 支持 PO 后续收货、发票录入 | 采购员、仓储人员、财务人员 | 录入部分收货与供应商发票 | 收货登记、发票登记、附件信息记录、数量和金额记录 | MySQL 存储收货单、发票、明细和附件元数据占位；MinIO 后段统一接入发票文件和收货凭证真实上传；暂不要求 RabbitMQ 事件发布 | 暂不涉及 | 未开始 |
