...
- 当前推荐实施切片：`RFQ 询报价`。它应只允许基于已通过审批的采购申请创建询价，继续复用 MySQL 和同步服务调用；报价附件先记录文件名、说明和元数据占位，不在本切片引入 MinIO 真实上传。
...
| P0 | RFQ 询报价 | 采购员对已审批申请发起询价并比较报价 | 采购员、供应商管理员 | 三家供应商报价，系统展示价格、交期、评分、风险 | RFQ 创建、供应商选择、报价录入、报价对比、推荐排序；附件先记录文件名/说明，暂不做真实上传 | MySQL 存储 RFQ、报价、供应商评分和附件元数据占位；MinIO 后段统一接入真实报价附件 | P1 接入报价推荐解释 | 建议下一步 |
