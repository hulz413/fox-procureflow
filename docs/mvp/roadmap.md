...
- 当前推荐实施切片：`AI 采购助手`。它应使用真实 DeepSeek API 补齐自然语言生成采购草稿、风险提示、报价解释和三单匹配异常解释；mock AI 不应作为最终行为。
...
| P1 | AI 采购助手 | 用真实 DeepSeek API 提升申请和决策效率 | 申请人、采购员、财务人员 | 自然语言生成采购草稿、风险提示、报价解释、三单匹配异常解释 | DeepSeek 接入、Prompt 模板、结构化 JSON 输出、调用审计、脱敏处理、用户确认后进入正式业务流程 | ai-service 封装 OpenAI-compatible client；MongoDB 存储 AI 会话、输入输出和审计记录；环境变量注入 API key | 使用真实 DeepSeek API，不以 mock 作为最终方案 | 建议下一步 |
