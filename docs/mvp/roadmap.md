...
- 当前推荐实施切片：`采购看板`。它应聚合已完成的申请、审批、RFQ、PO、收货、发票和三单匹配数据，提供集团和公司视角的 P1 管理看板；Redis 指标缓存和自然语言摘要可作为后续增强。
...
| P1 | 采购看板 | 展示集团和公司视角的采购状态 | 管理者、采购负责人、财务人员 | 查看待审批、采购金额、异常匹配、供应商分布 | 集团总览、公司筛选、采购金额趋势、待审批数量、异常匹配数量、供应商报价分布 | MySQL 聚合业务数据；Redis 指标缓存作为性能增强后段引入 | P1 可生成自然语言经营摘要 | 建议下一步 |
