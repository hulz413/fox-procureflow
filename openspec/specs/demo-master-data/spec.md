...
The frontend SHALL provide a read-only master data view that verifies the backend master data APIs in the procurement workspace.

#### Scenario: Open master data view
- **WHEN** a user opens the frontend master data view
...
- **WHEN** a user selects a demo company in the master data view
...
#### Scenario: Master data view remains read-only
- **WHEN** a user inspects the master data view
...
The system SHALL keep this change focused on master data and SHALL NOT implement downstream procurement workflows.
...
- **WHEN** a developer inspects the master data implementation
