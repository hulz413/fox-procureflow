# frontend-modularization 规格

## Purpose

定义 Fox Procureflow 前端模块化架构和行为保持契约，确保采购工作台从 `App.tsx` 单体拆分后，仍能稳定呈现星河控股集团多公司采购协同演示，并保留真实后端数据、公司隔离、共享供应商池、审批/订单/履约/三单匹配状态边界和本地 demo 验证路径。
